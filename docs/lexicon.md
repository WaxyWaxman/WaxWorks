...
| **multi-store** / **multiple stores** | The system serves more than one **Organization**, each with one or more **Stores**. Since 2026-09-20 the Organization is the tenant and the Store a dimension inside it: people belong to the Organization and are **assigned to** Stores ([E-01](flows/E-01-authenticate.md) d25, [M-04](flows/M-04-manage-users.md) d27); settings stay per Store ([M-06](flows/M-06-settings.md) d70); which records carry a Store and which are the Organization's is the architecture's to list — _its amendment to A-5 is proposed, not yet ratified_. Adjective is `multi-store`; the fact is stated as "the system is **multi-store**" or "serves **multiple stores**". | "multi-tenant" in prose (it *is* the mechanism, but the docs say multi-store), "multi-branch", "chain"; "multi-org" |
...
| **Manager** | Elevated role; a strict superset of Employee, and a strict subset of **Owner** ([M-04](flows/M-04-manage-users.md) d25 — which superseded d11 and d14's *no tier above Manager*). Holds a **PIN** for the manager-only line on a store session and may hold a **personal session**. Capitalised. | "admin", "supervisor"; using it for the Owner |
| **Owner** | The role above Manager: everything a Manager has, plus administering the **Organization** — its Stores and their **store accounts**, its Managers and Owners, assignment of anyone anywhere ([M-04](flows/M-04-manage-users.md) d25, d30; [M-06](flows/M-06-settings.md) d70). An Organization always has at least one active Owner (M-04 d26). Capitalised when the role is meant; the GL role *owner's equity* ([M-07](flows/M-07-chart-of-accounts.md) d31) is lowercase and unrelated. | "admin", "proprietor", "account owner"; using it for a Manager |
| **System Administrator** / **Sysadmin** | WaxWorks staff. Not a role *in* an Organization but the actor outside every one: creates an Organization and its first Owner, recovers an Organization that has lost its Owners, triggers password resets, and reaches **identity data only** — Organizations, Stores, people, the user-change log — never store data (`S-` series; [M-04](flows/M-04-manage-users.md) d25, d26). Signs in with a passkey ([E-01](flows/E-01-authenticate.md) d28). *Sysadmin* is the short form. | "the WaxWorks team" when this actor is meant; "admin", "support", "superuser" |
| **User** | An Employee, a Manager or an Owner — a person of one Organization, **assigned to** one or more of its Stores ([M-04](flows/M-04-manage-users.md) d27). Use only when the statement is true for all three roles. A System Administrator is not a User. | "account" (except in **store account**), "login", "operator" |
| **store assignment** / **assigned to** | The relationship between a User and a Store they may act at: on that Store's store session their initials and PIN resolve, and elsewhere they do not ([E-01](flows/E-01-authenticate.md) d25, d26; [M-04](flows/M-04-manage-users.md) d27). Many-to-many. Employees and Managers hold at least one; Owners may hold none. | "membership" (the deferred question's old name), "belongs to" (a User *belongs to* the Organization), "works at" |
| ~~**terminal enrollment** / **enroll**~~ | **Retired** by [E-01](flows/E-01-authenticate.md) d24, which superseded d9: a terminal now signs in with the Store's **store account** and holds a **store session**. Readable in superseded rows and in A-3 until that is amended. | using it for current behavior — say **store account** / **store session** |
| **store account** | A Store's own credential — its email and password ([E-01](flows/E-01-authenticate.md) d24). Set at the Store's creation and reset by an Owner and nobody else ([M-06](flows/M-06-settings.md) d70); rotating the password signs out every terminal of that Store. The one sense in which *account* is canonical. | "store login", "terminal account", "device account" |
| **store session** | What a terminal holds after signing in with the store account: shared, always-on, never lapsing, scoped to the Store and its Organization ([E-01](flows/E-01-authenticate.md) d24). The staff **session** below sits on top of it. | "enrollment", "terminal session", "kiosk mode" |
| **personal session** / **signed in as themselves** | A Manager or Owner signed in with their own email, password and second factor ([E-01](flows/E-01-authenticate.md) d27, d28). Nothing on it asks for initials or a PIN and nothing lapses; the person picks a Store and may switch. Employees have none. | "manager login", "named session", "admin mode" |
| **PIN** | A Manager's or Owner's four-digit credential for the manager-only line on a store session, typed on its own and unique among the Managers and Owners assigned to that Store ([E-01](flows/E-01-authenticate.md) d26). Set by an Owner or Manager, logged as changed never as a value ([M-04](flows/M-04-manage-users.md) d28). Employees have none. | "passcode", "code", "password" (a password is the personal session's) |
| **session** | Unqualified, the *staff* session: opened with initials on a **store session**, lapsing after a configured period of **inactivity, defaulting to 5 minutes** ([M-06](flows/M-06-settings.md) d45, [architecture](architecture.md) A-50) and taking **no maximum** ([E-01](flows/E-01-authenticate.md) d13). An **Open** Sale suppresses the lapse whatever the setting says (E-01 d10). Distinct from the store session beneath it, which does not lapse, and from a **personal session**, which has no lapse at all (E-01 d27). **There is no server-side staff session**: it is an actor and a timer in the browser ([architecture](architecture.md) A-3, A-50), so a deactivation ends it by the write path refusing the User, with the shell catching up at its next actor re-resolution ([M-04](flows/M-04-manage-users.md) d18, [architecture](architecture.md) A-55). Qualify it — *store session*, *personal session* — whenever the principal matters. | conflating it with the store session; "15 minutes", which was E-01 d4's flat figure (amended by M-06 d45) and is now neither flat nor the default |
...
| **manager-only** | An action an Employee cannot perform at all — a whole flow or function reserved to Managers and Owners (accounts payable, Undo End of Day, adjusting on hand, voiding a finalized Invoice). On a store session a Manager or Owner authorizes in place by entering their **PIN**, and both names are recorded; on a personal session nothing is asked ([E-01](flows/E-01-authenticate.md) d26, d27). Distinct from a **ReviewFlag**, which lets the Employee proceed and tells the Manager afterward, and from **Owner-only**, the short list only an Owner may do ([M-04](flows/M-04-manage-users.md) d30). | using it interchangeably with "review flag"; "manager override"; "by initials" for the mechanism |
| **Owner-only** | The short list reserved to Owners: creating a Store and holding its store account ([M-06](flows/M-06-settings.md) d70), creating, re-roling, deactivating or reassigning a Manager or an Owner ([M-04](flows/M-04-manage-users.md) d30). Everything manager-only is open to an Owner; the reverse is not true. | "admin-only", "superuser" |
...
| Canonical | Meaning | Avoid |
|---|---|---|
| **Organization** | The tenant: the company that runs one or more Stores. People belong to it and are **assigned to** its Stores; an Owner administers it; a System Administrator creates it ([M-04](flows/M-04-manage-users.md) d25–d27; `O-` and `S-` series). American spelling (§12). | "organisation", "company" as the entity name, "tenant" in prose, "account" |
| **Store** | One physical location, belonging to exactly one Organization. Since 2026-09-20 a dimension inside the tenant rather than the tenant itself: what is physical or operational — copies, Sales, Returns, CloseBatches, terminals, settings — carries a Store; what is the Organization's does not, and the architecture lists which is which (_amendment to A-5 proposed, not yet ratified_). Has a **store account** and a seven-digit Store ID the system assigns ([M-06](flows/M-06-settings.md) d47, d70). | "location" (except as [M-08](flows/M-08-general-ledger.md)'s journal-line dimension), "site", "branch", "tenant" in prose |
...
5. ~~**"Administrators"** appears in PRD goal G-2, where §2 of this lexicon says to avoid
   *admin* for the **Manager** role.~~ **Resolved 2026-09-20:** the only administrator in the
   model is the **System Administrator**, who never sees store data — so G-2 and G-4 cannot
   mean them. They mean Managers and Owners, and the PRD's next pass rewords them. Likewise
   E-05 d5's *(Admin)* and M-01's *Admin-only* are stale prototype language for manager-only.
...
**Entity forms follow §3's rule**, as §14 records: the modelled thing is PascalCase — `LedgerPosting`, `Reconciliation`, `SealedPeriod` — and the everyday word is lowercase: *a posting*, *a reconciled set*, *a sealed period*.
...
| **LedgerPosting** | The artifact a **Manager types by hand** — rent, utilities, a loan, depreciation, an owner's draw, tax ([M-08](flows/M-08-general-ledger.md) Phase 2, d47; `ledger_posting_create`). **Narrower than §14's *posting*,** which is any balanced set of debits and credits against GL accounts and includes the ones an artifact writes for itself ([M-07](flows/M-07-chart-of-accounts.md) d12, [architecture](architecture.md) A-67). The everyday word is *a posting*, and *a **typed** posting* wherever the distinction from an artifact's own journal matters — it is what makes [M-08](flows/M-08-general-ledger.md) d3's editability rule asymmetric | "manual ledger entry" (§14 — that is an accounts-payable row), "journal entry" for the typed one specifically |
| **Reconciliation** | Entries within **one account**, marked together against an outside document or against each other ([M-08](flows/M-08-general-ledger.md) d25, d37, d43; `ledger_reconciliations`). It **moves no money and gates nothing** — an unreconciled account seals exactly as a reconciled one does, so the mark is evidence, never a gate. A set never spans accounts. The everyday word is *a reconciled set*. **It has two kinds, and they are not two names for one thing** (d37): **`matched`** — offsetting entries, balance-neutral **by construction**, the two halves of an undeposited-funds movement; and **`cleared`** — the entries that appear on an outside document, a bank statement, which **is not required to net**, because the remainder is outstanding cheques and deposits in transit and is *the point of the exercise rather than a failure*. **§14's *clearing* redirect does not reach this.** That row sends **clearing** to *undeposited funds*, and it is about **naming an account**; applied to a **cleared set** it would collapse d37's two kinds into one and undo what building it found — a correct bank reconciliation refused, out by 18,800 | "clearing" for the act or the set (§14, and it is [M-05](flows/M-05-accounts-payable.md) d15's word for a payables act); "cleared" as a bare adjective on a single entry — it is always a **kind of Reconciliation** |
