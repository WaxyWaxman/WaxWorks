...
| **multi-store** / **multiple stores** | The system serves more than one **Organization**, each with one or more **Stores**. Since 2026-09-20 the Organization is the tenant and the Store a dimension inside it: people belong to the Organization and are **assigned to** Stores ([E-01](flows/E-01-authenticate.md) d25, [M-04](flows/M-04-manage-users.md) d27); settings stay per Store ([M-06](flows/M-06-settings.md) d70); which records carry a Store and which are the Organization's is [architecture](architecture.md) A-86's list (superseding A-5). Adjective is `multi-store`; the fact is stated as "the system is **multi-store**" or "serves **multiple stores**". | "multi-tenant" in prose (it *is* the mechanism, but the docs say multi-store), "multi-branch", "chain"; "multi-org" |
...
| **Store** | One physical location, belonging to exactly one Organization. Since 2026-09-20 a dimension inside the tenant rather than the tenant itself: what is physical or operational — copies, Sales, Returns, CloseBatches, terminals, settings — carries a Store; what is the Organization's does not, and [architecture](architecture.md) A-86 lists which is which (superseding A-5). Has a **store account** and a seven-digit Store ID the system assigns ([M-06](flows/M-06-settings.md) d47, d70). | "location" (except as [M-08](flows/M-08-general-ledger.md)'s journal-line dimension), "site", "branch", "tenant" in prose |
