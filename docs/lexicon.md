...
| **consumed** / **cleared** | The two terminal states of a credit, and they are different facts. **Consumed** means its value has been attached to debits — derived from the existence of a target row in a non-voided batch. **Cleared** means a Manager retired it from attention — derived from membership of a non-voided clearing, and **balance-neutral**. Neither is a stored column ([architecture](architecture.md) A-37). | "spent" / "used" / "closed"; storing either as a flag; treating them as one state |
