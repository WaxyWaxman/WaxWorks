...
**A claim void is a row, never a column** (A-44), in `supplier_claim_voids` with `unique(claim_id)` — the same rule and the same reason A-36 gives for `ap_payment_batch_voids`. A voided claim is therefore finished by the **presence of a row**, while an abandoned one is finished by the **status it carries** ([E-04](flows/E-04-manage-inventory.md) d24, d27); a query asking whether a claim is live has to consult both.
