...
**Immutability is enforced by trigger, and for an Invoice it releases.** **Paid** Invoices — not finalized ones (A-33) — SaleLines on Closed Sales, and voided Sale numbers raise on update or delete. An Invoice's trigger **evaluates the derived paid state above (A-33b); it does not read a stored flag**, so voiding the PaymentBatch that settled it ([M-05](flows/M-05-accounts-payable.md) d22, A-33a) releases it with nothing to flip. A trigger written to fire at finalize, or written so that it can never release, is wrong in both directions. Because the predicate spans tables, a path correcting an Invoice must lock its row and re-evaluate rather than trust a value read earlier in the transaction. Users, tax lines, and tender types are deactivated, never deleted.
