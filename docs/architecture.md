...
**Immutability is enforced by trigger, and for an Invoice it releases.** **Paid** Invoices — not finalized ones (A-33) — SaleLines on Closed Sales, and voided Sale numbers raise on update or delete. An Invoice's trigger is **conditional on its paid state, not on a one-way flag**: voiding the PaymentBatch that settled it ([M-05](flows/M-05-accounts-payable.md) d22, A-33a) takes it back out of Paid and it becomes correctable again. A trigger written to fire at finalize, or written so that it can never release, is wrong in both directions. Users, tax lines, and tender types are deactivated, never deleted.
...
| [architecture](architecture.md) §5.1 | The immutability trigger fires on **paid** Invoices and releases when an Invoice leaves Paid (A-33a) |
...
- **Is an Invoice's `paid` state stored or derived?** Nothing in §5 or §5.1 says. If derived (`paid ⟺ balance owing = 0`) the release in A-33a is automatic and the immutability trigger evaluates a correlated subquery per row; if stored, a void has to flip it and there are two sources of truth for the same fact. This has to be settled before the M3 trigger is written.
- **Where is `sum(claim applications) ≤ claim amount` enforced?** [M-05](flows/M-05-accounts-payable.md) d20 makes a SupplierClaim partially applicable and d22 makes an application reversible, but §5.1 carries no claim invariant at all. Two settlements each applying the last of a claim, concurrently, both succeed against a derived remaining. **A money invariant with no recorded enforcement point** — it needs a row lock taken by both the apply path and the void path. An invariant enforced only by the screen not offering the button is not enforced.
- **The Payables schema no longer matches what M-05 requires.** §5 derives an Invoice's balance from "`ap_payments` plus credited SupplierClaims", and the Payables group lists only `ap_payments` and `supplier_claims`. Four things have moved past it: d16 made the unit a **PaymentBatch** with typed **targets**; d19 put money and credit targets in the same batch; d20 means a claim contributes the amount applied **to that Invoice**, not its whole value; d22 means the view must **exclude voided batches**. Separately, the manual ledger entries of d12/d14 are a third input to a Supplier's balance with no table at all. §5's *principle* — balances derived from movements, never edited — is untouched and is precisely what makes a void clean; it is the **inputs** that are stale. **Drafted as a candidate A-33b and deliberately not written**, because it is a data-model decision binding flows not yet built, and the one above it has to be answered first.
