...
- **E-06 walked end to end on 2026-09-18** — T1–T7, prototype, port 5288, as E. Okafor (Employee) with Y. Nakamura and R. Delacroix as the Managers. Driven with the in-app browser rather than `playwright-cli`, which is not installed here, so **there is no recording to hand to `/qa automate`** — the first automate pass re-drives it. **Five rows conform outright: T2, T5, T6, T7**, and **T1 conforms but for its link default** (below). **Two findings and one gap:**
  ...
  - **Two open questions where the flow is silent.** (1) **A voided Return can still route its copy** — driven to completion on a `VOID` document whose money had been collected back, minting a sellable copy with no refund standing behind it. That is the shape of the loss decision 20 closed, reached from the other end, and **decision 20 speaks only of a Draft**. (2) Whether an **elapsed-but-unpromoted** change should still be badged and still be clearable — d66 anticipates the invisibility as an accepted consequence but does not settle the affordance. *Route:* `/flow-clarify E-06` and `/flow-clarify M-06`.
...
| M4 Till | E-05-T1–T3, T5–T16, E-06-T1–T17, E-07-T1–T7, E-04-T2, E-04-T4, E-04-T7, M-06-T1, T2, T4, T11 |
