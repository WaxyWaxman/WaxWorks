...
- **E-06 walked independently on 2026-09-18, by a session that wrote none of it.** T4 and T8-T14 driven for the first time, T1, T5, T6 and T7 spot-checked, all against the flow rather than against the code's intent. **Every row conforms.** T4 now mints where the earlier walk found it re-grading in place; T8 preselects `#100389` with the refund at **$31.49** rather than $34.99; T11's cap refuses the refund figure; T12 moves account **5410 Inventory adjustment — Damaged** by **$16.75** when assessed at $2.00 against an $18.75 cap, and leaves 5410 **unmoved** when assessed at exactly the cap; T13's Return of a copy sold under GST 5% charged **10%** while still defaulting the refund to the linked **$31.49**. **Two things the earlier walk could not verify now are:** T6's **password branch** — `RD` prompted, wrong password refused with *"Not that password."*, right password authorized, the Employee's session undisplaced — and T10's minted barcode, which **resolves at the till**, which is d18's substantive half.
  - **Finding — [E-06](../flows/E-06-process-a-return.md) decision 12's counter path does not exist.** d12 says *"Scanning the copy's own sticker resolves it outright."* **Add returned item** returns **No match** for an internal barcode — a seeded one, and one the re-grade had just minted — while listing those same copies under a title search, and Enter submits the dialog rather than resolving. The field searches catalogue text and manufacturer UPC only. **The till's own scan field resolves the same barcode**, so this is the Return screen alone, and it means every return goes through a title search. Also cited by **E-06-T1**, which says *scan the copy's sticker*. *Route:* the user — a prototype fix restoring d12.
