...
| E-06-T13 | **A Return charges today's rate, not the rate the Sale collected.** With a rate change taking effect between the Sale and the Return, the refund's tax resolves at **today's** rate. The refund *amount* still defaults to the linked line price, so the two deliberately behave differently — that contrast is what the row exists to hold. | 1, 4 | E-06 decision 13, E-06 decision 5, A-57 | As T1, plus a tax rate change effective between the Sale and the Return. **M-06's queued change did not promote** on the walk, so the rate was staged by editing the live one — [M-06](../flows/M-06-settings.md) ground, and the row's seed is not achievable through the queue today | Walked | — | |
