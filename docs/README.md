...
| [build/](build/) | How code gets built past the prototype: the [plan](build/plan.md) — where we are and what is next, read first — the [workflow](build/workflow.md), the [conventions](build/conventions.md) the developer codes to, the [build-status table](build/status.md), and the work orders |
