...
| [M-06](flows/M-06-settings.md) | Specified | Settings (`/settings`) | **Five of six configuration groups.** The three tracks: the group, the editor, and **the log** — which is the right track rather than an afterthought, because [architecture](architecture.md) A-52 is the decision this screen exists to honour: every write carries an actor, a timestamp, the key touched, and the values **before and after**. Manager-only in its entirety (A-28a), so the whole screen is gated on arrival and the authorising Manager's name is what every log row carries. **Sections** as an editable table (d28) with revenue, tracks-stock default, discountable, returnable and a dead-stock override (d20, d29, d30, d40) — Freight and Gift cards pre-loaded and marked system. **Tenders** as a table (d22, d23): many names, one behaviour, so `Visa` and `Mastercard` both settle as a card; `Cash rounding` is system-written (d26) and **refuses** to be renamed or switched off, naming the reason. **Currencies** with one planning rate each and the date it was last set, stamped on change rather than typed (d33, d38). **Store settings** — lapse, price ending, dead stock, stream aging, drawer policy, receipt width. **Store details** with the Store ID and position shown and unwritable (d47, [M-04](flows/M-04-manage-users.md) d11). **Not built:** tax, and the group says why — see below. |
...
- **Tax is still a flat rate per named line — M-06 d1's *superseded* shape.** d11 replaced it with two tables resolved through a Genre's product tax code (d12, d14), and the seed's blended `QC 14.975%` is the exact thing that replacement was for: it cannot report per tax type ([M-03](flows/M-03-daily-summary.md) d13), cannot carry a registration number per type (d48), and cannot do the by-rate split a period spanning a rate change needs (M-03 d15). `Genre` is a bare string, not an entity with a parent Section (d32) and a product tax code. The Settings screen lists Tax as a group and explains the gap rather than drawing an editor over the retired shape. Closing it means migrating the till's money path.
