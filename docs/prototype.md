...
mock in-memory store, and it resets on reload — except for **who signed in**, which
the terminal remembers the way the product's store-session token outlives a reload
([E-01](flows/E-01-authenticate.md) d24, [architecture](architecture.md) A-87).
...
| [E-05](flows/E-05-sell-a-record.md) | Specified | Point of Sale | Barcode resolver, multi-copy picker, customer pre-fill (with inline create-on-no-match), `0.00` prompt, negative inventory, split tender, hold/void, receipt, PO field, 2-digit discount. Also carries Edit (void-and-duplicate for Current, open for Held), Copy, Search (barcode → item sale history), and M-03's close under **Other functions** (View Subtotal / Total Today's Sales / Undo End of Day, Admin-labeled), now as the **eight-section report** ([M-03](flows/M-03-daily-summary.md) d17) — File info, Sales per Section code, Taxes with each tax's own base, Tenders/Misc naming the GL account and totalling **Undeposited funds** (d21), customer type, per hour, per Employee, and Tendering details. Sections 5–8 arrive **collapsed and are not remembered** (d28). Beside them, **This month / Last month / Year to date** run the same report over a date range (d27), carrying seven sections and not the listing. The seeded month's batches store their summaries with A-83's schema version, so the range report has something to sum. Laid out as three tracks that own the window (d29): a **till rail** that collapses to five icons and opens as a drawer over the sale (d30), the sale itself, and a money rail with **Finish sale** pinned to its floor. **Void** only at zero (d31) — the money must be refunded, moved to the Customer's account, or struck first, and striking a settled tender really does put it back (d32). |
...
| [M-08](flows/M-08-general-ledger.md) | Specified — **ships after the first release** (d46) | Ledger (`/ledger`) — five phases behind one manager gate: **Opening position**, **Postings**, **Seal a period**, **Read the books**, **Reconcile** | 36 decisions, all five phases wired to the shared store. **Every rule lives in `src/lib/ledger*.ts` and none of it in the screen** — A-74 moves M-08's invariants into the definer function on A-4 and A-48's terms, so the screen collects input, calls a `*Refusal()` and renders what comes back. The opening position derives equity rather than taking it (d26) and materialises it as a line only at the seal (A-78); a posting shows the amount still needed and on which side, and is refused unbalanced (d10); a period is **sealed**, never closed (d4), unsealed only at the most recent and only with a reason (d18, d29); a statement carries **provisional** and a stored issuance keeps that mark after its period seals (d36). Suspense, retained earnings, *Accounts payable* and the gift card liability are not offered; Inventory and the tax accounts are (d13, d33, d34) |
| [O-01](flows/O-01-administer-the-organization.md) | In clarification | Organization (`/organization`) | **Owner-only in its entirety** (d1): the manager-only line and then a second question — a Manager who crosses is refused **by name**. Stores with Store ID, position, trading name, store account email and the count assigned; **New Store** mints the next seven-digit ID and position, starts the Store from M-06's defaults with nobody assigned and logs it on the Organization (d2, M-06 d70); **Reset store account** warns that every terminal of the Store is signed out — *including this one* when it is — and does exactly that (d3, E-01 d24), after which the old password is refused. Owners and Managers link into Users; the Organization log closes the screen. **No figure of any Store** appears (d4). **Not built:** closing a Store, the Organization's own name and details, copy-from-sibling — all open in the flow. |
| [S-01](flows/S-01-onboard-and-recover-organizations.md) | In clarification | Administration (its own shell; reached only by a System Administrator) | **Identity data only** (d1, A-90): Organizations; Stores' identity — the `stores_identity` view; people without PIN or password; the change log over `organizations.log`, `stores.log` and `users.log`. The shell has **no route to any Store screen**, and asking for one lands here. **Create an Organization** with its first Owner, invited, with no Store (d2). **Recover** reactivates a deactivated Owner or invites a new one, and is **refused while the Organization has an active Owner**, naming them (d3, A-90). **Send reset link** for an Owner logs on the person and on the Organization, where its Owners read it (d3, O-01 d5). Sign-in is a passkey, which the prototype fakes as a button and says so (d4). **Not built, and not buildable in memory:** the read-path boundary — here it is the screen's; the product's is a Postgres role's. |
...
- **The Organization model is carried by swapping Store slices, not by keyed reads.**
  [A-86](architecture.md) makes the Organization the tenant and the Store a dimension;
  this in-memory prototype keeps the **active Store's** per-Store data flat in state
  exactly as before and parks every other Store's slice, swapping them when the
  principal's Store changes (`loadStore` in `AppStore.tsx`). Two Stores therefore hold
  genuinely separate stock, Sales, closes, settings and counters, and the user-visible
  rule — one Store at a time ([E-01](flows/E-01-authenticate.md) d27) — is the shape of
  the state. What it cannot show is anything that reads across Stores: the **Supplier
  balance** is derived from the Invoices in the slice, so the same Supplier shows a
  smaller debt at the Plateau counter than at the home one — the exact consequence A-86
  names and answers with the Organization-wide select policy on Invoice headers, which
  the prototype does not model. Do not let a walk assert a Supplier balance across
  Stores against it. The catalog is per Store (A-86); the Plateau Store is seeded from
  the same base catalog for review's sake and holds no stock.
- **Every credential is a fake and says so.** Store account passwords, personal
  passwords and PINs are plain strings in `seed.ts` and are **printed on the sign-in
  screen**; the second factor is the constant `000000`; the passkey is a button. The
  rules under test are real — which door exists for whom, one refusal wording, the PIN
  verified in the store on every crossing, a miss that names nobody, a clash that names
  nobody — and the credentials are not ([A-87](architecture.md), A-89, A-91).
- **The System Administrator's boundary is the screen's, not the read path's.** The
  administration shell routes to nothing of any Store, and the Gate never mounts a
  Store screen for that principal; but the in-memory context still holds everything,
  and a console can read it. [A-90](architecture.md)'s Postgres role is what the product
  enforces; the prototype demonstrates the scope of the surface only.
- **Signing a terminal out is signing this browser out.** *Every terminal of the Store*
  ([E-01](flows/E-01-authenticate.md) d24, [O-01](flows/O-01-administer-the-organization.md)
  d3) reaches exactly one terminal here. The product revokes the store account's
  refresh tokens; the prototype clears its remembered principal.
