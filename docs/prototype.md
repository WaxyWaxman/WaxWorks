...
| E-01, M-06 | — | *stubbed* | E-01's session is shell behaviour rather than a screen — the identification prompt, the lapse, and the manager-authorize dialog. `CURRENT_USER` is still a constant, and `ManagerOverride` still carries the retired name under a corrected title. |
