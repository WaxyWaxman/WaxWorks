...
| **LedgerPosting** | A balanced set of debits and credits a **Manager types by hand** — rent, utilities, a loan, depreciation, an owner's draw, tax ([M-08](flows/M-08-general-ledger.md) Phase 2). Manager-only ([architecture](architecture.md) A-74), refused if it does not balance (d10) and refused into a sealed period (d11). **Not** a *manual ledger entry*, which is an accounts-payable row ([lexicon](lexicon.md) §4) |
