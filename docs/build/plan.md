...
**Last updated:** 2026-09-21, sr-talbot's session
...
Zero flow orders exist. **M0 is built and merged; its order is `In review` with
28 findings recorded and none yet fixed in code.**
...
| [#178](https://github.com/WaxyWaxman/WaxWorks/pull/178) | **open** | A-104 (the owner's ceiling), A-105 (functions in `app`, tables in `public`, `app` exposed), A-91 amended (sign-up off) |

**What runs today.** From a clean clone with Node 24, pnpm 11, Docker Desktop and
the Supabase CLI 2.117.0 on PATH: `pnpm install`, `pnpm typecheck`, `pnpm lint`,
`pnpm test` (307 repository assertions), `supabase start -x studio,imgproxy,inbucket,edge-runtime,logflare,vector,realtime,storage-api,mailpit,supavisor`,
`supabase db reset`, `supabase test db` (8 assertions), `pnpm --filter @waxworks/web build`.
A Vercel project (`pressers/wax-works`) deploys an access-protected preview per
pull request; it runs the app shell against **no database**, because no Supabase
account exists yet.
...
1. **Merge #178.** It carries the decisions step 2 builds to.
2. **One fix pull request against the M0 order** — `/develop docs/build/orders/M0-foundation.md`, interactively, on a branch from `main`. It carries, in this order:
   - a **correcting migration** (A-101: the first is never edited): `alter role waxworks_app bypassrls`; `revoke usage on schema app from anon`; a comment recording that the first migration's closing comment described the alternative A-104 rejected;
   - two **pgTAP suites**: every function in `app` is owned by `waxworks_app`; `app` holds functions only (A-104, A-105);
   - `supabase/config.toml`: `[api].schemas` gains `app`; `enable_signup = false` (A-105, A-91);
   - `packages/contracts/src/supabase.ts`: `.schema("app").rpc` (A-105);
   - `packages/contracts/src/wrapper.ts`: the wrapper **mints** `p_request_id`; the input schema drops it (A-94; Findings 4 and 12);
   - `resolve_scan` gains `p_code`; the six §6-stated return shapes replace `z.unknown()` in `helpers/` (Findings 14, 15);
   - `tests/contracts.test.ts` walks file → list as well as list → file (Finding 21);
   - `apps/web/instrumentation.ts`: `onRequestError` removed until M1 lands A-93's scrubber, or the test extended and row 17 amended (Findings 8, 20, 22);
   - `e2e/package.json` gains `typecheck` (Finding 16); `.gitignore` widens to `.env*` + `!.env.example` (Finding 9); `apps/web/package.json` drops the stray `cn` and moves `shadcn` to dev (Finding 26);
   - each Finding's Developer cell marked `fixed` or `disputed`.
3. **`/work-order check docs/build/orders/M0-foundation.md`** again on that pull request: the reviewers re-check the findings only (workflow §3 step 5). Then **`/work-order accept`**: the Verdict, the **Needs a human** list, and row 13's deferral written into `architecture.md` §11 as an open question so it cannot be dropped. A human merges. The coordinator writes M0's row in [status.md](status.md).
4. **The M1 contract pull request** (A-99): input and output schemas and the fake's behaviour for M1's functions — the Identity set in §6 — plus the answers to the three header questions the M0 review left open (Findings 6, 24: `actor_resolve`, `manager_authorize_pin`, `terminal_register` and the **S** functions cannot take the uniform header; the two predicates; the nine `settings_*` groups). Both humans review; `/plan-check` first if any answer touches a recorded decision.
5. **`/work-order E-01 D`** and **`/work-order E-01 U`**. Human approves each. Dispatch the `developer` subagent per order, in its own worktree, with the order's path and nothing else.
6. **`/qa ready M1`** when both are `Accepted` and merged, then `/qa automate` per E-01 register row.
