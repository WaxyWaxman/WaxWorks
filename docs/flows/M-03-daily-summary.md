...
- **A day *before* the books open is protected by nothing, and stays undoable** ([M-08](M-08-general-ledger.md) d40). The rule above is about a **sealed** period; step 16 of M-08 only ever offers months from the opening position forward, so a day earlier than that can never be sealed and Undo End of Day stays available on it indefinitely. **This is deliberate rather than an omission:** d40 puts those days' journal lines outside what any statement reads, so restating one moves no reported figure. It would stop being harmless the moment a statement began reading them, which is the thing d40 forecloses.
