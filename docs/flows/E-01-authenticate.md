...
| 19 | **Identification resolves as you type: no Enter, no OK button, and Escape cancels the action rather than just the dialog.** The counter is the constraint. Decisions 12 and 15 make opening a Sale, recording a pay-out, adjusting on hand and voiding prompt **every time, session or not**, so this interaction runs many times an hour and has to cost a keystroke or two. **The moment what has been typed can only be one active User, that is who it is** and the action proceeds. **Resolution is by prefix, not exact match:** `R` is enough where R. Delacroix is the only active person whose initials start with R; `RD` becomes necessary the day R. Mbeki is hired. An exact hit still wins outright where it is also a prefix of somebody longer, which decision 14's two-character minimum keeps rare. **Ambiguity waits rather than guessing** — two candidates resolve to nobody and the prompt asks for another character — so the failure mode is **a keystroke, never a misattribution**. **Only active Users resolve.** A deactivated User cannot be typed at all, which is the same refusal decision 17 and [architecture](../architecture.md) A-55 put in the write path, arriving one layer earlier. **The prompt confirms with the person's name, never by echoing the letters back** — initials are reusable ([M-04](M-04-manage-users.md) d16), so the letters are not the confirmation. **Deliberately not a picker.** A list of names to click would put every prompt behind a read-and-aim and would show the whole staff list to whoever is standing at the counter; where a prefix is ambiguous the prompt shows the competing **initials**, not the people. *Accepted consequence, and it is the one to watch:* **the keystroke that worked yesterday can stop working tomorrow**, because the staff list changed rather than anything the Employee did — hiring somebody whose initials share a first letter silently lengthens what a colleague has to type. It fails safe and it will still be irritating, and nothing warns the Manager at the moment of hiring that they have just done it |
