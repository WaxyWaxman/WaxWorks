...
3. **Creates an Organization** (decision 2): a name, and the first Owner's name and email — **no initials** (decision 5): the Owner has no Store yet for them to resolve at, and takes them when first assigned ([M-04](M-04-manage-users.md) d34). The Owner receives an invite and sets their own password and second factor ([M-04](M-04-manage-users.md) d29, [E-01](E-01-authenticate.md) d28). The Organization has no Store yet — its Owner creates the first one ([O-01](O-01-administer-the-organization.md) d2). There is no public sign-up.
...
5. Beyond recovery, a System Administrator **edits nothing about a person** in this version — what *edit users* should cover is an open question below, and the answer is not built until it is decided (S-01 d1's boundary is the default).

---

## Requirements

- A System Administrator reaches identity data only, and the boundary is enforced where the data is read, not by which screens exist (decision 1; [architecture](../architecture.md) A-90 — a distinct Postgres role holding exactly those grants).
...
| 5 | **Onboarding takes the first Owner's name and email and nothing else — not their initials. Decided 2026-09-20 (walk finding); [M-04](M-04-manage-users.md) d34 receives it.** Initials are what a counter resolves ([E-01](E-01-authenticate.md) d19, d25), and an Owner created here has no counter: no Store exists until they create one ([O-01](O-01-administer-the-organization.md) d2), and nobody is assigned to it until they assign them. Asking WaxWorks staff to invent a stranger's initials at onboarding puts the disambiguation [M-04](M-04-manage-users.md) d13 gives a Manager — who knows the floor — in the hands of someone who has never met the shop. So the Owner takes initials at their first assignment, and Assign refuses until they have them (M-04 d34). *Accepted consequence:* an Owner who wants to ring a Sale on day one has an extra step, and the refusal at Assign is what tells them |
...
- **Closing an Organization.** Nothing here removes one. _Recommended, not yet ratified:_ deactivate, never delete, mirroring the recommendation for a Store in [O-01](O-01-administer-the-organization.md); its store accounts and personal sessions stop signing in; its data is retained under whatever the PRD decides for retention ([PRD](../PRD.md) §5's open question) — and its logs and its books with it, since [architecture](../architecture.md) A-95 binds a log's life to its record's and gives the books their own horizon.
- **`owner_recover` while an active Owner exists** — **refused** ([architecture](../architecture.md) A-90): an Owner who exists is the one to act.
...
- ~~**Where does the administration area live?**~~ — **Resolved by [architecture](../architecture.md) A-90:** a route group of the same application; the isolation is the principal's, not the URL's.
