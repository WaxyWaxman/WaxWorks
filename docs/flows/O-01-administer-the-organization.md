...
6. **Closing a Store, and the Organization's own name and details, are not in this version** — both are open below, and nothing is built for either until they are decided.
...
| 6 | **The store account is rotated here and nowhere else; the Settings screen shows the account's email read-only. Completes decision 3; [M-06](M-06-settings.md) d70 keeps the account as a fact of the Store. Decided 2026-09-20 (walk finding).** M-06 d70 put the store account in Store details and decision 3 put its rotation on this screen, and the walk found the two documents pointing at different surfaces. The act is an Owner's kill switch over every terminal of a Store, not a setting a Manager tunes — so it lives with the other Owner-only acts, once, where the log of it sits beside the Store's creation. Settings displays the email so a Manager reporting a problem can read it out (M-06 d47's reasoning for the Store ID), and offers no control. *Accepted consequence:* an Owner at a Store's Settings who wants to rotate that Store's account leaves the screen; one surface for one act was judged worth the walk |
