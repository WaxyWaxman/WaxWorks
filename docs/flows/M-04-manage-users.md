...
The design intent is that Employees have high agency. The manager-gated list below is short on purpose, and everything not on it is available to any Employee. **User administration itself is on that list** ([architecture](../architecture.md) A-55) — it was omitted until A-55 found it, which, read against decision 2, made promoting yourself an Employee action.
...
|---|---|---|
| **Void or amend** a finalized Invoice | [E-02](E-02-receive-inventory.md) step 23 and E-02 d40, [E-04](E-04-manage-inventory.md) | Restates what was received and owed |
...
| **Add** a User | [architecture](../architecture.md) A-55 | Creates an actor the system will attribute work to |
| **Change** a User's role | this flow, [architecture](../architecture.md) A-55 | Grants or removes Manager. Refused where it would leave no active Manager (decision 14) |
| **Deactivate** or **reactivate** a User | this flow, decisions 15 and 19 | Removes or restores access. Deactivation is refused where it would leave no active Manager (decision 14) |
...
| **Reactivate** | Restores the original User row, so one person keeps one history (decision 19). Prompts for new initials where theirs have been reused. |
| **Password** | A Manager may set, change or clear one — optional for anyone, up to 8 characters (decision 23, [E-01](E-01-authenticate.md) d21). Logged as changed, never as a value. |
| **Correct** | A Manager may fix a User's initials or display name at any time (decision 22). Uniqueness and normalisation still apply, the change is logged before-and-after, and it shows retroactively wherever attribution is displayed. |
...
| **Deactivate** | Removes access **immediately at the write path** — no new work can be attributed to them from the instant it commits, and each terminal's shell drops the session at its next actor re-resolution (decisions 15 and 18). An Open Sale stays finishable. Their initials are **released** for a new hire (decision 16). Refused where it would leave the Store with no active Manager (decision 14). **Users are never hard-deleted** — every Sale, adjustment, and authorization stays attributed to the person who performed it, so the record has to survive their leaving. |
...
- Adding, re-roling, deactivating and reactivating a User are manager-only ([architecture](../architecture.md) A-55).
...
- Below-cost pricing and Invoice adjustments beyond ±2% raise review flags (E-02 d35); voiding a finalized Invoice remains manager-only.
...
- **Credentials** — **deferred to a later version**, not open. Decision 7's *no invite step* holds for as long as [E-01](E-01-authenticate.md) d3 does; when credentials arrive, Add grows an issuance step and whether Managers get stronger ones than Employees is decided then. [E-01](E-01-authenticate.md) d18 carries the checklist of what has to be revisited at that point, and this flow is on it.
- **Multi-store membership** — **deferred to a later version**, not open. Whether a User can belong to several Stores, whether a Manager can administer more than one, and how switching works. Three things reopen when it arrives: decision 13's per-Store initials uniqueness, decision 14's *at least one active Manager per Store*, and [E-01](E-01-authenticate.md) d8's one-Store User.
