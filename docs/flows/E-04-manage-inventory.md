...
**Status:** Specified
**Related:** [E-02 Receive inventory](E-02-receive-inventory.md) · [E-03 Search the inventory](E-03-search-inventory.md) · [E-05 Point of Sale](E-05-sell-a-record.md) · [M-02 Re-order inventory](M-02-reorder-inventory.md) · [M-05 Accounts payable](M-05-accounts-payable.md) · [M-07 Chart of accounts](M-07-chart-of-accounts.md)
...
Price-ending rounding — nearest instance of the store's one configured ending, as a suggestion only — applies to shelf prices set here as it does at receiving ([E-02](E-02-receive-inventory.md) d55, decision 31 below).
...
- **Editing a Record's genre may offer to re-pull its tags from the catalog provider** ([M-06](M-06-settings.md) d54). The tags a Record was adopted under are a snapshot ([architecture](../architecture.md) A-61), so a correction made months later is otherwise read against what the provider said then. The re-pull is **offered, never automatic**, touches only this Record in this Store, and **does not re-run the map** — M-06 d53's resolve-once rule is unchanged, and the matched tag is never re-resolved. If the provider is unreachable the edit still saves and only the refresh fails, following [E-03](E-03-search-inventory.md) d8's rule for search.
...
- **Void and amend are refused while any live PaymentBatch targets the Invoice** ([M-05](M-05-accounts-payable.md) d37). This flow's void-and-amend route already excluded a **fully** paid Invoice ([architecture](../architecture.md) A-41); M-05 d37 extends the refusal to a **partly** paid one, so the order is forced — void the payment in [M-05](M-05-accounts-payable.md) first. The refusal names what is holding the Invoice, so it is discoverable rather than a dead end.
...
| 8 | ~~`.50`/`.99` rounding applies to shelf prices set here (E-02 decision 9)~~ — **superseded by decision 31** (one configured ending, nearest, still a suggestion) |
...
| 31 | **A shelf price set here is pre-filled to the nearest instance of the store's one configured price ending, and only ever as a suggestion. Records [architecture](../architecture.md) A-49 and [M-06](M-06-settings.md) d44, following [E-02](E-02-receive-inventory.md) d55; supersedes decision 8 and leaves decision 17 as it stands.** The same two moves E-02 d55 records — two endings to one, always-up to nearest — and the same thing left alone: decision 17's *suggestion, not a rule*. Appended to close the gap [architecture](../architecture.md) §9 already recorded as made against this flow |
...
- **Can a correction here ever land inside a sealed period?** [M-08](M-08-general-ledger.md)'s
  ...
  **The likely answer is that it cannot**, because [M-07](M-07-chart-of-accounts.md) d8
  ...
  saying so; if it does not, this flow needs a refusal. **Raised by the [M-08](M-08-general-ledger.md) audit**, which found that neither flow mentions M-08 or a sealed period at all. **Deferred with M-08** ([M-08](M-08-general-ledger.md) d46, #78): the ledger ships after the first release, and until it lands `period_is_sealed()` returns **false** for every day ([architecture](../architecture.md) A-80), so in the first release there is no sealed period for a correction to land in. The question waits with the flow that owns it.
...
- ~~**Re-grading a copy after it is sellable.**~~ — **Dissolved by [E-06](E-06-process-a-return.md) decision 15, not answered here. Closed 2026-09-19 (#97) as no longer a valid question.** E-06 d15 settles the case that raised it: re-grading on a Return **mints a new InventoryItem**, and *the copy that sold stays sold at the grade it sold at* — so no completed Sale ever has a grade change underneath it, and the data question this bullet described cannot arise by that route. Editing a copy's grade while it is still on the shelf is ordinary and needs no rule. *Original:* Editing a copy's grade is permitted, but a copy that was sold at a grade it no longer carries is a data question the snapshot rule sidesteps rather than answers.
...
- ~~**Bin location**~~ — **Resolved** by [E-03](E-03-search-inventory.md) decision 23, which owns it: **out of scope for v1**, a Section is as fine as location gets, and a bin is an added field on a copy the day it is wanted.
