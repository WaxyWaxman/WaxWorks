...
- **Tax rate changes over time — narrowed to the future-dated half, which is the whole of what is left.** **Closed:** whether history needs protecting. It does not need a new mechanism — [architecture](../architecture.md) A-57 and §5 record that a Sale line snapshots *the rates applied*, not a reference to them, so editing a rate in place restates nothing; A-52's `settings_log` keeps the before-and-after; and decision 8 holds. **No versioning table is wanted for the past**, and A-57 settles the two cases that straddle a change — a Held Sale re-resolves at tender ([E-05](E-05-sell-a-record.md) d33) and a Return charges today's rate ([E-06](E-06-process-a-return.md) d13). **Still open, and it is an operational question rather than a data-integrity one:** a legislated rate takes effect at an announced date and time, and *editing in place* means somebody has to do it at that moment. A shop opening at ten whose rate changed at midnight trades the first hours at the old rate, and the snapshot then faithfully records the wrong thing — correctly, which is worse, because there is nothing to detect. The candidate shapes are unchanged: **effective-dated rates**, which A-57's refusal of versioning-for-history expressly does not rule out, or **a new tax type with the old deactivated** (decision 9's shape), which keeps each rate a separate row and separates them in [M-03](M-03-daily-summary.md)'s per-type reporting at the cost of a type list that grows with every legislative change. **A third question rides on the choice and should be settled with it:** whether a reporting period spanning a change must break tax out **by rate** within a type, which the two shapes answer differently for free.
