...
| **First name**, **Last name** | Both required. *Was a single **Name** field until decision 30.* |
| **Business** | Optional — the company on a Business account; the person named above is its contact. |
...
| 30 | **A Customer carries a first name and a last name, an optional business name, and an optional phone and email — the single Name field is split. Decided 2026-09-19 (#90).** The *Sorting the list* question had three ways out and this takes the second: **split the field**, which the question feared would leave every Business card with an empty family name — answered by making **Business** a field of its own beside the person's name, so a Business account carries its contact and its company as two facts rather than one string. Sorting therefore falls to **Last name**, then First name — *the sort key follows from the field shape rather than being named in the decision*. Decision 14's search covers both name fields and the business name. *Accepted consequence:* every card, Business accounts included, now names a person; a company with no contact to name gets the person who opened the account. The prototype's `Customer.name` is one string and has to split to follow |
...
- **Data protection.** The record holds name, address, email, phone, and purchase history. Retention, export, and deletion-on-request obligations are unaddressed. Two things are settled around it ([architecture](../architecture.md) A-93, A-95): none of those fields enters an operational log or error event — an event names a Customer by Primary ID and `org_id` and nothing else — and whatever deletion-on-request comes to mean, it reaches the record's `customer_ledger` and any log the record comes to carry, at the same moment and never before.
