...
7. Acting on a catalog-only row — ordering it, stocking it, editing it — **pulls it into the local catalog**, auto-filling every field the catalog provider provides and prompting for the ones it cannot supply (supplier, **genre**, and anything else store-specific). **Section is not among them** — it is derived from the genre's required parent ([M-06](M-06-settings.md) d31, d32), so it is never prompted for and never stored beside the genre. A row that has not been pulled in yet therefore shows **tags and no genre**, which is what distinguishes it from a Record the shop adopted and never stocked (decision 17).
