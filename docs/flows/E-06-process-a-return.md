...
- **May a copy that was never sold be taken as a return line at all?** The lookup shows every copy with its state
  (decision 12) and accepts one that is `sellable`. Nothing says whether it may be *taken*, and the independent walk
  found that when it is, the copy stays on the shelf — the Return says *"off the shelf until assessed"* while the
  titlecard still lists it sellable, so cash can leave the till for a copy that never left the floor. Decision 6 and the
  Requirements above are contradicted by that, but only because a case nobody sanctioned is reachable.
