...
- **Where does an Employee see copies waiting to be routed?** Decision 20 creates a state the flow did not have
  before: a **finished** Return whose copy has not been assessed. The copy is off the shelf and not yet anywhere
  else, and the customer has gone. Nothing says whether that is a list on the till, a badge on the Return in the
  rail's Recent, a worklist like [E-02](E-02-receive-inventory.md)'s receiving queue, or simply the Employee
  remembering. It is the same shape as decision 18's unstickered copy and may want the same answer.
