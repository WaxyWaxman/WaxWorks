...
- **Does an unmatched Return charge tax at all?** Decision 26 makes the money a **purchase** rather than a reduction of
  revenue, and a purchase from a private individual is not a taxed sale — but the till still resolves tax on the line and
  hands it over, so a `$34.99` disc bought in over the counter costs the shop `$40.23`. The close reports that tax,
  because [M-03](M-03-daily-summary.md) decision 14 reports **every movement of money** and it genuinely moved; whether
  it should have been charged is a different question and this is not it. Settling it changes what the customer is
  handed. *Owned by this flow, with [M-07](M-07-chart-of-accounts.md) for where any input credit would post.*
