...
- **A Sale's tender records the behaviour, not which configured tender it was. Raised by building [M-07](M-07-chart-of-accounts.md) Phase 3.** [M-06](M-06-settings.md) d22 gives `Visa` and `Mastercard` separate accounts for a stated reason — *"they settle as separate deposits, and a breakdown that merges them cannot be tied back to a bank statement"* — and [M-07](M-07-chart-of-accounts.md) d21 keeps one account per tender on that same reasoning. **The till's tender pad offers the seven behaviours**, so a Sale says `Credit Card` and nothing more, and at the close every card in the day posts to whichever tender account is listed first. The reconciliation d22 exists to protect is not reachable from a Sale. Two shapes answer it and both are this flow's to choose: the pad offers the **configured tenders** rather than the behaviours, or a Sale records which row it resolved through alongside the behaviour. Nothing is lost today — [M-03](M-03-daily-summary.md)'s breakdown is per behaviour and always was — so this is a gap that only costs something once a journal posts.
- **A pay-out cannot be journalled correctly from what a Sale records. Raised by building [M-07](M-07-chart-of-accounts.md) Phase 3, and this flow owns it.** Decision 16 and the [lexicon](../lexicon.md) agree on what a pay-out is: **cash removed from the till for an expense**, appearing as a negative cash line in the [M-03](M-03-daily-summary.md) close. The correct ledger entry is therefore **debit the pay-out expense, credit the cash it came out of**, and [M-07](M-07-chart-of-accounts.md) d21 provisions that expense account, calling it *"the one kind no reserved account covered"*. **What a Sale stores is the opposite shape.** A pay-out is a negative tender that leaves the Sale owing, so the till requires an offsetting tender to fund it — a $20 pay-out on an otherwise empty Sale is recorded as `Pay-out -20` **and** `Cash +20`. The Sale therefore carries $20 of cash that never entered the drawer, and **nothing on it says which tender was the offset**, so the right entry is not derivable. On a Sale that also sold $36.20 of records, Cash posts a $56.20 debit where $16.20 moved, and the expense posts a **credit** where a debit belongs. **The journal balances exactly** — the signed tenders sum to the Sale's total by construction — so [M-07](M-07-chart-of-accounts.md) d10's Suspense stays at zero and two accounts are wrong by twice the pay-out. This is the failure M-07's *"An imbalance and a wrong tender are different failures"* describes, except that this one **is** detectable, because a Pay-out tender is visible on the Sale; M-07 detects and reports it at the close rather than guessing. Two shapes answer it, both this flow's: record **which tender funded** the pay-out, or store it as the **negative cash line** decision 16 already calls it, with no offsetting tender at all.
