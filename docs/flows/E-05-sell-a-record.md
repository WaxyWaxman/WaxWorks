...
| 37 | **Merging an Open Sale into a Held Sale is not built in the first pass. Decided 2026-09-19 (#81).** It was never blocking — opening the hold and ringing the extra copies onto it reaches the same end state — so the three things the operation would have to answer (which lock survives under decision 23, which identifier survives under decision 3, and what becomes of the absorbed Sale's log) wait until someone needs the operation rather than the outcome. [E-07](E-07-manage-customers.md) d20, which raised it, is unchanged |
| 38 | **A Customer may be detached from an Open Sale; on a Held Sale the same control offers Cancel Hold instead, because a hold belongs to the customer it is held for. Decided 2026-09-19 (#108).** The till has always done the first half and no row has ever said so — decision 20 records that *a Sale need not have a Customer attached*, which permits the **state** and names no act. **What detach undoes is asymmetric, and that follows from rules already written rather than from an oversight.** The Customer's **global discount stays on the lines**: [E-07](E-07-manage-customers.md) d6 *pre-fills* it, step 3 says it pre-fills *"and remains editable per line"*, and decision 13 makes every line value a **snapshot** — so a pre-filled discount is the line's own figure and detaching does not reach back for it. The Customer's **tax group falls away**: tax resolves live from two axes, *the Customer's tax group **or, with no Customer, the store's default tax group***, so the fallback is the rule doing what it says. Whoever detaches mid-Sale should know the money moves on one axis and not the other. **On a Held Sale detach is refused, and the refusal is a prompt rather than a dead end.** A hold exists *for* somebody: decision 7 gives it no expiry, and [E-07](E-07-manage-customers.md) d19 makes the Customer's account track the one place *"a hold nobody has chased"* becomes visible, listing Held Sales per Customer under d17's **Waiting** band. A detached hold would sit on the shelf forever, in **no** Waiting band, with no contact preference to chase — [M-02](M-02-reorder-inventory.md) d32's failure mode reached from the other side and worse, since an attached stale hold is at least visible on somebody's card. Decision 6 already has the right verb, **Cancel Hold**, *"a distinct action from voiding a Sale, and logged rather than erased"*, so the control offers it in place of the refusal — the same move [M-02](M-02-reorder-inventory.md) d43 makes for an order line, and for the same reason: reaching for the wrong verb should surface the right one. *Accepted consequence:* **a Held Sale cannot change hands.** A hold rung up against the wrong customer is cancelled and re-rung rather than re-pointed, which costs the hold reference (decision 3) and its log |
| 39 | **A gift card balance does not expire. Redemption is the only way it drains. Decided 2026-09-20 (#80).** Balances already persisted indefinitely and nothing said whether that was the design or an omission; it is the design. **[M-08](M-08-general-ledger.md) d33 has been resting on this and says so:** its lock on the gift card liability *"is safe only while redemption is the sole way the balance drains"*, and its open question names this flow as the owner because *"whoever adds an expiry will not be reading this file"*. This row is that statement, made where the cards live. **It also keeps [A-51](../architecture.md) honest:** a GiftCard is a code and a movement history whose balance is *"the sum of its movements and is stored nowhere"*, so an expiry would have to be a **movement** — a system-written drain against a card nobody presented — and there is no act in this flow that writes one. Nothing expires because nothing would do the expiring. **The customer-facing reason is the plain one:** a card is a thing somebody paid for and physically presents at the counter, and the shop honours what it says. *Accepted consequence, and it is a real cost carried deliberately:* **the liability never ages out.** A card sold today and never used is still a liability in fifteen years, still in [M-08](M-08-general-ledger.md) d33's computed figure, still on the balance sheet, and it grows with every card nobody redeems. The alternative — expiring balances into income — would flatter the books by taking money the shop was paid to hold |
...
- ~~**Gift card expiry.**~~ — **Resolved** by decision 39: **balances do not expire**, and redemption is the only drain. This is what [M-08](M-08-general-ledger.md) d33's liability lock has been resting on, and its open question named this flow as the owner. *Original:* Balances currently persist indefinitely; several jurisdictions regulate this.

- **Escheatment is undecided, and decision 39 does not answer it — it is what makes the question live.** With no expiry, an unredeemed balance sits as a liability for as long as the shop exists. Several jurisdictions treat unclaimed property of that kind as **remittable to the province** after a dormancy period, and if Alberta does, three things follow that nothing here specifies: what counts as dormant, what act removes the balance from the card (it cannot be a redemption — nobody presented it — and [A-51](../architecture.md) has no other movement kind), and where the money posts, since paying it out is neither revenue nor a redemption ([M-08](M-08-general-ledger.md) d33's figure would have to stop counting it). **This is a question for someone qualified on Alberta's unclaimed-property position, not a product decision**, and it is recorded rather than guessed. *It does not block anything:* until the answer is known the balance simply stays on the card, which is decision 39's behaviour and the conservative direction.
- ~~**Merging a Sale in flight into a Held Sale.**~~ — **Deferred**, not built in the first pass (decision 37, #81). *Original:* Raised by [E-07](E-07-manage-customers.md) d20, and deliberately left open there. Folding an Open Sale's lines into an existing hold is a new operation, and it has to answer three things the current rows do not: which lock survives (decision 23 locks an Open Sale to whoever opened it), which identifier survives (decision 3 gives a Held Sale an `H`-reference replaced by a Sale number on tender), and what becomes of the absorbed Sale's log. Nothing is blocked meanwhile — opening the hold and ringing the extra copies onto it reaches the same end state.
