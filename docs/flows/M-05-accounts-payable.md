...
6. When a supplier confirms credit for a claim ([E-04](E-04-manage-inventory.md)), the Manager marks it **Credited**. Applying it is then a deliberate act: the Manager **selects the outstanding Invoices it should settle and enters how much lands on each** (decision 18). Nothing is distributed automatically. The **total is not the Manager's to choose** (decision 23): a credit goes out in full, or to the limit of what the supplier is owed, whichever is smaller. Where it exceeds everything owed, the excess is emitted as a **remainder Credit** against that supplier and the claim is finished (decisions 24, 25) — a claim is never left part-applied.
...
- A claim is applied **whole or not at all** (decision 24). It is never left part-applied, and carries no running remainder.
...
- **What a remainder Credit clears against.** Decision 25 retires a remainder from attention using decision 15's clearing, but decision 15 takes **manual ledger entries only** and requires the set to sum to zero. A remainder absorbed by subsequent *Invoices* has no entry to pair with, so either clearing widens to admit it or a remainder is retired some other way. Not decided; a remainder nets correctly either way, so this is tidiness rather than arithmetic.
