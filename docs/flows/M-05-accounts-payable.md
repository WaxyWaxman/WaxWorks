...
- **A void that unfreezes an Invoice, followed by a correction that shrinks it.** Decision 22 returns an Invoice to correctable; [E-02](E-02-receive-inventory.md) may then reduce its total below what *other, still-live* PaymentBatches already paid against it — a negative balance owing, which decision 8 forbids absorbing by editing the balance. The overpayment question below is about a payment exceeding a balance; this is a balance shrinking beneath its payments, and neither has a landing place.
- **Does a claim come back onto the list when a void refills it?** Decision 20 says a claim leaves the outstanding list when its remaining reaches zero, phrased as a one-way departure. A void makes remaining positive again, and decision 22's "in hand to be applied again" implies the claim returns — but nothing says so, and nothing says anyone is told. Decision 20's own accepted consequence is that a part-applied credit is a new way for money in the store's favour to be quietly forgotten; a credit that silently reappears is exactly that.
...
- **Where `sum(applications) ≤ claim amount` is enforced.** Decision 20 makes a claim partially applicable and decision 22 makes an application reversible, but nothing states where the total is bounded. Two settlements each applying the last of a claim, concurrently, both succeed against a derived remaining. This is a money invariant with no recorded enforcement point — it belongs in [architecture](../architecture.md) §5.1, and is recorded there as an open question.
