...
- A claim may only be applied against an Invoice from the same supplier. Which of that supplier's Invoices, and how much lands on each, is the Manager's choice; the **total is not** (decisions 18, 23).
- A **Credited** claim counts against the balance immediately; a **Pending** claim and a **Claim placeholder** do not (decisions 26, 14). The distinction is whether the supplier has agreed, not how the credit was raised.
...
- **Voiding a payment whose target has moved on — amended, voided, or cleared.** Decision 22 returns an Invoice to **Finalized** so it can be corrected. The reverse order is undecided, and there are three doors into it: an Invoice paid then **amended** or **voided** in [E-04](E-04-manage-inventory.md); and a manual-entry target since **cleared** under decision 15. What the returned money attaches to when the thing it settled no longer says what it said is unanswered in all three cases.
- **A void that unfreezes an Invoice, followed by a correction that shrinks it.** Decision 22 returns an Invoice to correctable; [E-02](E-02-receive-inventory.md) may then reduce its total below what *other, still-live* PaymentBatches already paid against it. Decision 25 gives that overpaid amount somewhere to go — a remainder Credit, with the balance allowed to go negative — but nothing yet says the correction **emits** one, and decision 8 still forbids editing the balance to absorb it.
...
- **May a re-record pre-fill from the voided batch?** Decision 22 makes a partly-wrong payment a void-and-re-record, which means retyping the targets that were right. Pre-filling the replacement from the voided batch is the obvious ergonomic fix and is in **direct tension with decision 18**, which refuses a pre-filled claim split on the grounds that a pre-filled split is decision 11 under another name. Whichever way it goes, it should be written down.
- **A duplicated bank reference.** After a void and a re-record, `Cheque 101` appears on both a voided batch and a live one. Decision 5 makes the reference the thing that reconciles against the bank statement, and that surface now shows the same cheque twice — harmless if the voided state is visible there, confusing if it is not.
...
- **Overpayment.** Nothing defines what happens when a payment exceeds an Invoice's balance. Decision 25 now gives money in the store's favour an artifact and allows a negative balance, so the machinery to answer this exists — whether an overpayment should emit a remainder Credit the way an excess claim credit does is the obvious reading, and is **not yet decided**.
