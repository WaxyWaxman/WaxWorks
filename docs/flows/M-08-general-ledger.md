...
14. **Retained earnings and *Accounts payable* are not offered** (decision 13, corrected by [M-07](M-07-chart-of-accounts.md) d31 — *current profits* is derived and is not an account). **Suspense is not offered either**, and is cleared by its own act (decision 14, not ratified). **Inventory is offered** — a dead-stock write-down is exactly the case where book value should diverge from the shelf.
...
| 32 | **The general ledger is out of scope for the first build and moves to the second iteration. Decided 2026-09-19 (#70).** Nothing is retracted: the chart and the artifact journals [M-07](M-07-chart-of-accounts.md) owns are built and stay. What this flow adds on top — the opening position, typed postings, sealing, and the statements — is not built until the second iteration, and the register's M-08 rows wait with it. **Consequences accepted.** Decisions 3 and 14 stay *not yet ratified*; every open question below stays open, **deferred with the flow rather than answered** (#71–#78) — and so do the two other flows carry on its behalf: [E-02](E-02-receive-inventory.md) and [E-04](E-04-manage-inventory.md)'s *can a correction land inside a sealed period* (there is no sealed period in the first build for one to land in) and [M-07](M-07-chart-of-accounts.md)'s *operating versus non-operating* (there is no P&L to draw). Phases 2–4 remain *drafted, not interrogated* and must be walked with `/flow-clarify M-08` before the second iteration builds them. The status line stays `Specified` only because the vocabulary has no *deferred* value; this row, [PRD](../PRD.md) §7 and [architecture](../architecture.md) §8 are where the deferral is recorded |
...
**Deferred with the flow.** Decision 32 moves this flow to the second iteration (#70), so everything below stays open until then — deferred, not answered. The record of what was asked stands as written.
