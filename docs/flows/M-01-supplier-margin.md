...
- **The billing address is now consumed** ([M-05](M-05-accounts-payable.md) d17): accounts payable displays it as the remit-to address alongside the payment being recorded. Decision 13's "nothing consumes either address yet" is no longer true of billing; shipping is still waiting on [E-04](E-04-manage-inventory.md) claim correspondence. This card stays the only place either is edited, so a stale billing address is corrected here and nowhere else.
- **Payment terms are read from the Invoice, not from the card.** Decision 19 makes this field a *default*; [E-02](E-02-receive-inventory.md) d45 copies it onto each Invoice and lets receiving override it, and accounts payable ages the Invoice's value. Changing terms here therefore affects future receiving only — the same shape as decision 2's rule for Discount.
...
| 19 | **A Supplier carries payment terms — `Net 15` / `Net 30` / `Net 45` / `Net 60` / `On receipt` / `COD` / `Prepaid` — as the default terms for Invoices received from them.** A new field constraining nothing existing. It is **not Cancel-by**, which this card already carries and which is an *ordering* figure: days from order-placed to auto-cancel an unfulfilled order. Nothing on this card, or anywhere in the system, previously recorded when a bill falls due, which is why [M-05](M-05-accounts-payable.md)'s aging question could not be answered — the field was missing, not the report. The Invoice carries its own terms defaulted from here and overrides them where the paperwork differs ([E-02](E-02-receive-inventory.md) d45). Terms are an ordinary field on the card: they save on change and are not gated, unlike Discount (decision 13). *Accepted consequence:* **Prepaid and COD are not aging terms**, so the field admits two different kinds of thing — one that produces a due date and one that does not. What it means for a Supplier on those terms to carry an outstanding balance is left open in [M-05](M-05-accounts-payable.md) rather than answered here |
| 20 | **A Supplier carries a default payment method, and the terms list gains `Net 90` and `End of Month`.** **Extends decision 19**, which added payment terms but nothing about *how* the store pays — a gap only visible once [M-05](M-05-accounts-payable.md) had a settlement to pre-fill. Method is one of `Cheque` \| `Credit Card` \| `EFT` \| `e-Transfer` \| `Cash` \| `Other`. **EFT and e-Transfer are kept apart**: they are different rails that appear differently on a bank statement, and [M-05](M-05-accounts-payable.md) d5 makes that statement the reconciliation surface, so collapsing them would lose the distinction at exactly the moment it is needed. **`End of Month` means the last day of the month the invoice is dated in** — 3 September and 28 September are both due 30 September. *Accepted consequence:* a late-month invoice on those terms is due almost immediately, which is a real property of the term and not a defect to design around; and **`Other`** is a bucket that reconciles against nothing, so it is offered because an unusual method needs somewhere to go, not because it is a good answer. Both fields are ordinary card fields — they save on change and are not gated, unlike Discount (decision 13) |
