...
| `docs/build/` | The build plan (`plan.md` — where we are, what is next, read first when picking up), the workflow (`workflow.md`), the stack conventions the developer codes to (`conventions.md`), the build-status table (`status.md`), and the work orders (`orders/`) |
